...
type Widget struct {
	...
	IsRecrring     bool      `json:"is_recurring"`
...
func (m *DBModel) GetWidget(id int) (Widget, error) {
	...
	widgets
	where id = ?`, id)
	err := row.Scan(&widget.ID, &widget.Name, &widget.Description, &widget.InventoryLevel, &widget.Price, &widget.Image, &widget.IsRecrring, &widget.PlanID, &widget.CreatedAt, &widget.UpdatedAt)
	if err != nil {
		return widget, err
	}
...
// InsertCustomer inserts a new order, and return its id.
